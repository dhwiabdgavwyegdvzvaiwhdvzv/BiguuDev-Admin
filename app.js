...
// Mirrors scriptserver_worker.js's ASN_GRACE_WINDOW_SECONDS (3h) and
// ASN_GRACE_WINDOW_TIGHT_SECONDS (30min, once suspectedSharing is true).
// Keep these two values in sync if the backend constants ever change.
const ASN_GRACE_WINDOW_SECONDS = 3 * 60 * 60;
const ASN_GRACE_WINDOW_TIGHT_SECONDS = 30 * 60;

function networkChangeCell(s){
    if(!s.pendingAsn) return "-";
    const windowSeconds = s.suspectedSharing ? ASN_GRACE_WINDOW_TIGHT_SECONDS : ASN_GRACE_WINDOW_SECONDS;
    const deadline = (s.pendingAsnSince || 0) + windowSeconds;
    const remaining = deadline - Math.floor(Date.now() / 1000);
    const remainingLabel = remaining > 0
        ? `auto-locks in ~${Math.ceil(remaining / 60)} min if it doesn't switch back`
        : `grace window expired — will lock on next request`;
    return `New ASN detected${s.pendingCountry ? ` (${s.pendingCountry})` : ""} — ${remainingLabel}`;
}
...
.onclick=loadScripts;
// Local screen-lock only — NOT the real security boundary (that's the
// ?key= ADMIN_KEY in the URL, checked server-side on every API call). This
// just keeps the dashboard from being glanced at if someone picks up an
// unlocked device with the URL already open. The admin sets their own PIN
// on first use; only its SHA-256 hash is ever stored, never the PIN itself
// — so there's nothing here that can leak from reading the page source.
const AUTH_STORAGE_KEY = "biguudev_admin_auth";
const PIN_HASH_STORAGE_KEY = "biguudev_admin_pin_hash";
...
async function attemptLogin(){
    ...
    if(!hasPinSet()){
        // First time — whatever is entered here becomes the PIN going
        // forward (only its hash is stored).
        localStorage.setItem(PIN_HASH_STORAGE_KEY, await sha256Hex(pin));
        grantAccess();
        return;
    }

    const enteredHash = await sha256Hex(pin);
    if(enteredHash === localStorage.getItem(PIN_HASH_STORAGE_KEY)){
        grantAccess();
    }else{
...
function attemptReset(){
    // "Forgot PIN" — this is a screen-lock, not the real security boundary,
    // so resetting it just clears the local PIN (the person still needs the
    // real ?key= URL to see any data at all). No password to "reveal".
    localStorage.removeItem(PIN_HASH_STORAGE_KEY);
    showToast("PIN cleared — set a new one on next login");
    grantAccess();
}
...
const sharingBadge = user.suspectedSharing ? `<span class="badge risk" title="${user.weeklyAsnChanges||0} network changes in 7 days">SHARING?</span>` : "";
const pendingBadge = user.pendingAsn
    ? `<span class="badge" title="New network detected — auto-locks if it doesn't switch back in time">NETWORK CHANGE PENDING</span>`
    : "";
const ipSummary = user.ip1 || "-";
return `
<tr>
<td data-label="Username">${user.username} ${lockBadge}${riskBadge}${sharingBadge}${pendingBadge}</td>
...
<tr><td>Device Claimed</td><td>${s.hasDeviceClaim ? "Yes (HMAC-bound)" : "Not yet — next request claims it"}</td></tr>
<tr><td>Shadowrocket Device</td><td>${s.uaDevice||"-"}</td></tr>
<tr><td>Network Change</td><td>${networkChangeCell(s)}</td></tr>
<tr><td>Suspected Sharing</td><td>${s.suspectedSharing ? `<span class="badge risk">YES — ${s.weeklyAsnChanges||0} network changes in 7 days</span>` : "No"}</td></tr>
...
function convertLegacyScript(code){
    ...
    // The relay shell already resolves LAST_APP_UA (with its own request-header
    // fallback) and sends it as input.appUA — so this one specific call can be
    // safely auto-replaced. Any *other* persistentStore key has no server-side
    // equivalent at all (it's client-device-only storage) and can't be
    // mechanically converted — flag it instead of guessing.
    out = out.replace(/\$persistentStore\.read\(\s*["']LAST_APP_UA["']\s*\)/g, "input.appUA");

    if(/\$persistentStore/.test(out)){
        warnings.push("Detected $persistentStore usage beyond LAST_APP_UA. persistentStore only exists on-device in Shadowrocket — there's no server-side equivalent. If this reads/writes some other key, that logic needs to move into the relay shell itself (buildRelayShell in scriptserver_worker.js) and be forwarded as a new input.* field, similar to how appUA works.");
    }

    // Both $response.body (UI-modifying, http-response rules) AND
    // $request.body (quota/consume-logic, http-request rules) map to the
    // same input.body — the relay shell auto-detects which one Shadowrocket
    // actually provides at runtime, so the source script only ever needs
    // input.body regardless of which rule type it's meant for.
    out = out.replace(/\$response\.body/g, "input.body");
    out = out.replace(/\$request\.body/g, "input.body");
    out = out.replace(/\$request\.url/g, "input.url");
    // Bracket-notation variants — same meaning, different syntax.
    out = out.replace(/\$response\s*\[\s*["']body["']\s*\]/g, "input.body");
    out = out.replace(/\$request\s*\[\s*["']body["']\s*\]/g, "input.body");
    out = out.replace(/\$request\s*\[\s*["']url["']\s*\]/g, "input.url");

    // Destructuring can't be safely auto-converted (would need to know the
    // bound variable name and reconstruct the assignment) — flag it instead
    // of silently leaving broken $response/$request references behind.
    if (/\{[^}]*\b(body|url)\b[^}]*\}\s*=\s*\$(response|request)/.test(out)) {
        warnings.push("Detected destructuring from $response/$request (e.g. `const { body } = $response`). This can't be auto-converted — rewrite it manually as `const body = input.body;` / `const url = input.url;`.");
    }

    // Terminal $done({ body: EXPR }); -> return EXPR;
    out = out.replace(/\$done\s*\(\s*\{\s*body\s*:\s*([\s\S]*?)\s*\}\s*\)\s*;?/g, "return $1;");

    // Shorthand form $done({ body }); (property name === variable name,
    // no colon) — easy to miss since it looks nothing like the colon form
    // above, but it's extremely common in early-exit / error branches.
    out = out.replace(/\$done\s*\(\s*\{\s*body\s*\}\s*\)\s*;?/g, "return body;");

    // Any OTHER shorthand $done({ someVar }); — we can't safely assume
    // someVar is meant to be the body; flag it instead of guessing.
    if (/\$done\s*\(\s*\{\s*[a-zA-Z_$][\w$]*\s*\}\s*\)/.test(out)) {
        warnings.push("Detected a shorthand $done({ someVariable }) call where the variable isn't named `body`. Auto-convert can't safely assume what this should become — rewrite it manually as `return someVariable;` if that's the intended output.");
    }
